...
from util.learning_utils import StatefileCapableLearner, ModelWriter, TbWriterHelper, GamesIterator
...
class Learner(StatefileCapableLearner):
    ...
    def run_with_demand_files(self):
        ...
        for g_number, game in enumerate(GamesIterator('demand')):
            ...
            validation_sequences = drain_generator(
                DemandCustomerSequence(cfg.DEMAND_FORECAST_DISTANCE, validation[0], validation[1],
                                       flatten_sequences=True), 10)
